...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
        <a href="{{ route('vehicles.index') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
            {{ __('common.nav_vehicles') }}
        </a>
        <span>/</span>
        <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $vehicle->full_name }}</span>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" id="vehicle-page" x-data="{
        ...
        <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="w-full h-full flex items-center justify-center">
                            <svg class="w-16 h-16 text-gray-300 dark:text-gray-600" fill="none"
                                ...
                                <path stroke-linecap="round" stroke-linejoin="round"
                                      d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3
            ...
            <div class="flex items-start justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $vehicle->full_name }}</h1>
                    @if($vehicle->category)
                        <span class="inline-flex mt-2 items-center px-3 py-1 rounded-full
                                     text-sm font-medium bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">
                ...
                <div class="text-right">
                    ...
                    @if($dailyRate)
                        ...
                        <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('vehicles.per_day') }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                <h2 class="font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('vehicles.specifications') }}</h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    @foreach($specs as $spec)
                        <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                            <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">{{ $spec['label'] }}</p>
                            <p class="text-sm font-semibold text-gray-900 dark:text-gray-100 mt-0.5">{{ $spec['value'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            @if($vehicle->features && count($vehicle->features) > 0)
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                    <h2 class="font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('vehicles.features') }}</h2>
            ...
            @if($vehicle->description)
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                    <h2 class="font-bold text-gray-900 dark:text-gray-100 mb-3">{{ __('vehicles.about_vehicle') }}</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{{ $vehicle->description }}</p>
                </div>
            @endif

            {{-- Availability --}}
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                <h2 class="font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('vehicles.availability') }}</h2>
                <div class="flex flex-wrap gap-3 text-xs mb-4">
                    <span class="flex items-center gap-1.5 text-gray-600 dark:text-gray-300">
                        <span class="w-3 h-3 rounded bg-green-100 dark:bg-green-900/60 border border-green-300 dark:border-green-700"></span>
                        {{ __('vehicles.available') }}
                    </span>
                    <span class="flex items-center gap-1.5 text-gray-600 dark:text-gray-300">
                        <span class="w-3 h-3 rounded bg-red-100 dark:bg-red-900/60 border border-red-300 dark:border-red-700"></span>
                        {{ __('vehicles.booked') }}
                    </span>
                </div>
        ...
        <div class="space-y-5">
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 sticky top-24">
                <h2 class="font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('vehicles.book_this_vehicle') }}</h2>

                @auth
                    @if(auth()->user()->role === 'admin')
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
                            <div class="flex items-center gap-2">
                                <svg class="w-5 h-5 text-yellow-600 dark:text-yellow-400 flex-shrink-0" fill="none"
                            ...
                            <p class="text-xs text-yellow-700 dark:text-yellow-400 mt-1 ml-7">
                    ...
                    @else
                            ...
                            <div>
                                ...
                                <input type="text" id="v-pickup-date" placeholder="{{ __('vehicles.select_date') }}" readonly
                                       class="w-full text-sm border border-gray-200 dark:border-gray-700 rounded-lg
                                              ...
                                              focus:ring-indigo-500 bg-white dark:bg-gray-800 dark:text-gray-100 cursor-pointer">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400
                                              uppercase tracking-wide mb-1.5">
                            ...
                            <template x-if="pricing && !loadingPrice">
                                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-gray-600 dark:text-gray-300">{{ __('vehicles.duration') }}</span>
                                        <span class="font-medium text-gray-900 dark:text-gray-100"
                                              x-text="`${pricing.price?.days ?? 0} {{ __('vehicles.days') }}`">
                                        </span>
                                    </div>
                                    <div class="flex justify-between text-sm">
                                        <span class="text-gray-600 dark:text-gray-300">{{ __('vehicles.calculation') }}</span>
                                        <span class="font-medium text-right text-xs text-gray-900 dark:text-gray-100"
                                              x-text="pricing.price?.breakdown ?? '—'">
                                        </span>
                                    </div>
                                    <div class="border-t border-gray-200 dark:border-gray-700 pt-2 flex justify-between">
                                        <span class="font-bold text-gray-900 dark:text-gray-100">{{ __('vehicles.total') }}</span>
                                        <span class="font-bold text-indigo-600 dark:text-indigo-400 text-lg"
                                              x-text="pricing.price
                                                  ? 'AFN ' + new Intl.NumberFormat().format(pricing.price.amount)
                                                  : '—'">
                                        </span>
                                    </div>
                                    <div x-show="pricing && !pricing.available" class="mt-2 text-xs text-red-600 dark:text-red-400 font-medium">
                        ...
                        <div x-show="pricing && pricing.available && selectedFrom && selectedTo" x-cloak class="mb-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div x-show="paymentMethod === 'cash'" x-cloak
                                     class="p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800
                                            rounded-xl text-xs text-orange-700 dark:text-orange-400">
                                    ⚠️
                                    {{ app()->getLocale() === 'fa'
                            ...
                            <div x-show="paymentMethod === 'bank_transfer'" x-cloak
                                 class="mt-3 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl space-y-3">
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-lg p-3 text-xs space-y-1.5 border border-blue-100 dark:border-blue-900">
                                    <p><span class="text-gray-500 dark:text-gray-400">{{ app()->getLocale() === 'fa' ? 'نام بانک:' : 'Bank Name:' }}</span>
                                        <strong class="text-gray-900 dark:text-gray-100">Afghan United Bank</strong>
                                    </p>
                                    <p><span class="text-gray-500 dark:text-gray-400">{{ app()->getLocale() === 'fa' ? 'نام حساب:' : 'Account Name:' }}</span>
                                        <strong class="text-gray-900 dark:text-gray-100">Morwarid Car Rental</strong>
                                    </p>
                                    <p><span class="text-gray-500 dark:text-gray-400">{{ app()->getLocale() === 'fa' ? 'شماره حساب:' : 'Account Number:' }}</span>
                                        <strong class="font-mono text-gray-900 dark:text-gray-100">1234-5678-9012</strong>
                                    </p>
                                    <p><span class="text-gray-500 dark:text-gray-400">{{ app()->getLocale() === 'fa' ? 'شعبه:' : 'Branch:' }}</span>
                                        <strong class="text-gray-900 dark:text-gray-100">Dasht-e-Barchi, Kabul</strong>
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <input type="text" x-model="bankReference" placeholder="e.g. TRF-2026-123456"
                                           class="w-full text-sm border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-lg
                            ...
                            <div x-show="paymentMethod === 'mastercard'" x-cloak
                                 class="mt-3 p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-xl space-y-3">
                                ...
                                <div>
                                    ...
                                    <p id="stripe-card-errors" class="text-xs text-red-600 dark:text-red-400 mt-1"></p>
                                </div>
                        ...
                        <form id="booking-form" method="POST" action="{{ route('customer.bookings.store') }}">
                            ...
                            <button type="submit" id="booking-submit-btn" :disabled="!canSubmit"
                                    :class="canSubmit
                                        ? 'bg-indigo-600 hover:bg-indigo-700 cursor-pointer'
                                        : 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed'"
                                    class="w-full py-3 text-white text-sm font-bold
