            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($vehicles as $vehicle)
                    ...
                    @if($vehicle->status === 'available')
                        <div class="px-4 py-3 bg-green-50 flex items-center justify-between border-t border-gray-100">
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-sm text-green-800 font-medium">Dasht-e-Barchi, Kabul</p>
                            </div>
                            <a href="https://maps.google.com/?q=Dasht-e-Barchi+Kabul"
                               target="_blank"
                               class="text-xs text-green-700 underline font-medium">Maps →</a>
                        </div>
                    @elseif(in_array($vehicle->status, ['booked', 'active']))
                        <div class="px-4 py-3 bg-orange-50 border-t border-gray-100">
                            <p class="text-sm font-semibold text-orange-700">Currently on a rental trip</p>
                            @if($vehicle->last_seen_at && $vehicle->last_seen_at->gt(now()->subMinutes(10)))
                                <p class="text-xs text-orange-500 mt-0.5">
                                    🚗 {{ $vehicle->last_speed }} km/h
                                    · Updated {{ $vehicle->last_seen_at->diffForHumans() }}
                                </p>
                            @endif
                        </div>
                    @else
